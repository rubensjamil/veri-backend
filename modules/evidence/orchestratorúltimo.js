...
// ============================================================
const CNPJS_FAMOSOS = JSON.parse(fs.readFileSync(path.join(__dirname, 'cnpjs_famosos.json'), 'utf8'));
console.log('📦 CNPJS_FAMOSOS carregado. UFs: ' + Object.keys(CNPJS_FAMOSOS).length);

const VERSAO_ORQUESTRADOR = '4.0.9';
...
// ============================================================
// EXECUTA BUSCAS POR PRIORIDADE (OTIMIZADO)
// ============================================================
async function executarBuscas(queries, nome) {
    var criticas = await Promise.all([
        buscarGoogleComAbort(queries.google, TIMEOUTS.GOOGLE_SEARCH),
        buscarGoogleComAbort(queries.news, TIMEOUTS.NOTICIAS),
    ]);

    var complementares = await Promise.allSettled([
    ...
    var google = criticas[0] || null;
    var noticias = criticas[1] || null;

    var processos = complementares[0].status === 'fulfilled' ? complementares[0].value : null;
    var reclameData = complementares[1].status === 'fulfilled' ? complementares[1].value : null;
    var consumidorData = complementares[2].status === 'fulfilled' ? complementares[2].value : null;
    var protestos = complementares[3].status === 'fulfilled' ? complementares[3].value : null;
...
async function cadeiaDeBuscaCNPJ(limpo) {
    ...
    if (brasil) {
        try { await carregarHistorico(); salvarNoHistorico(cnpjNormalizado, brasil); } catch(e) {}
    ...
    if (receita) {
        try { await carregarHistorico(); salvarNoHistorico(cnpjNormalizado, receita); } catch(e) {}
...
async function coletarEvidenciasReais(nome, cnpj, cpf, uf, modulo, subModulo) {
    ...
    // 0. BANCO LOCAL (PRIORIDADE MÁXIMA)
    ...
    // 1. BUSCA CNPJ POR NOME (se não tiver CNPJ e não achou no banco)
    ...
    // 2. FORÇA BUSCA CADASTRAL (BrasilAPI) MESMO COM CNPJ DO BANCO
    ...
    // 5. SITE OFICIAL – via Google com fallback
    ...
    // 7. RASTREABILIDADE
    var rastreabilidade = {
        google_search: { sucesso: resultados.google !== null, itens: resultados.google ? resultados.google.length : 0 },
        reclame_aqui: { sucesso: resultados.reclameData !== null, itens: resultados.reclameData ? resultados.reclameData.length : 0 },
        noticias: { sucesso: resultados.noticias !== null, itens: resultados.noticias ? resultados.noticias.length : 0 },
        processos_judiciais: {
    ...
    // 9. ADICIONA SITE AOS DADOS CADASTRAIS
    ...
    // 10. RETORNA – COM faturamento_anual GARANTIDO
    var resultado = {
        fontes: fontes,
